...
void UploadAttribute(GLuint* id, int numElements, int dataSize, int attribSize, int attribID, void* pointer, const string&debugName) {
	...
	memcpy(ptr, pointer, numElements * dataSize);
...
Mesh* Mesh::GenerateBunchaTriangles() {
	Mesh* m = new Mesh();
	m->numVertices = 5;

	m->vertices = new Vector3[m->numVertices];

	m->vertices[0] = Vector3(-0.5f, 0.25f, 0.0f);
	m->vertices[2] = Vector3(0.0f, 0.25f, 0.0f);
	m->vertices[1] = Vector3(-0.25f, -0.25f, 0.0f);
	m->vertices[3] = Vector3(0.0f, -0.75f, 0.0f);
	m->vertices[4] = Vector3(0.5f, 0.25f, 0.0f);
	

	m->colours = new Vector4[m->numVertices];
	m->colours[0] = Vector4(rand()/((float)RAND_MAX+1), rand() / ((float)RAND_MAX + 1), rand() / ((float)RAND_MAX + 1), 1.0f);
	m->colours[1] = Vector4(rand() / ((float)RAND_MAX + 1), rand() / ((float)RAND_MAX + 1), rand() / ((float)RAND_MAX + 1), 1.0f);
	m->colours[2] = Vector4(rand() / ((float)RAND_MAX + 1), rand() / ((float)RAND_MAX + 1), rand() / ((float)RAND_MAX + 1), 1.0f);

	m->colours[3] = Vector4(rand() / ((float)RAND_MAX + 1), rand() / ((float)RAND_MAX + 1), rand() / ((float)RAND_MAX + 1), 1.0f);
	m->colours[4] = Vector4(rand() / ((float)RAND_MAX + 1), rand() / ((float)RAND_MAX + 1), rand() / ((float)RAND_MAX + 1), 1.0f);

	m->BufferData();
	return m;
}

Mesh* Mesh::GenerateQuad(bool big) {
	Mesh* m = new  Mesh();
	m->numVertices = 4;
	...
	if (!big) {
		m->vertices[0] = Vector3(0.5f, 1.0f, 0.0f);
		m->vertices[1] = Vector3(0.5f, 0.5f, 0.0f);
