...
void OGLRenderer::SetShaderLight(const vector<Light> &l) {
	memset(directions, 0, sizeof(float) * 12);
	memset(positions, 0, sizeof(float) * 12);
	memset(specColours, 0, sizeof(float) * 16);
	memset(EmissColours, 0, sizeof(float) * 16);

	for (size_t i = 0; i < 16; i++) {
		size_t offset = i % 4;
		size_t dim = i / 4;
		float colPart1 = l[dim].GetSpecColour()[offset];
		specColours[i] = colPart1;
		float colPart2 = l[dim].GetEmissColour()[offset];
		EmissColours[i] = colPart2;
	}

	for (size_t i = 0; i < 12; i++) {
		size_t offset = i % 3;
		size_t dim = i / 3;
		float colPart1 = l[dim].GetPosition()[offset];
		positions[i] = colPart1;
		float colPart2 = l[dim].GetDirection()[offset];
		directions[i] = colPart2;
	}


	for (int i = 0; i < 4; i++) {
		radii[i] = l[i].GetRadius();
		angles[i] = l[i].GetCutOff();
	}

	glUniform3fv(glGetUniformLocation(currentShader->GetProgram(), "lightDirections"), 12, directions);

	glUniform3fv(glGetUniformLocation(currentShader->GetProgram(), "lightPose"), 12, positions);

	glUniform4fv(glGetUniformLocation(currentShader->GetProgram(), "specColours"), 16, specColours);

	glUniform4fv(glGetUniformLocation(currentShader->GetProgram(), "emissColours"), 16, EmissColours);

	glUniform1fv(glGetUniformLocation(currentShader->GetProgram(), "lightRadii"), 4, radii);

	glUniform1fv(glGetUniformLocation(currentShader->GetProgram(), "lightAngles"), 4, angles);
}
...
void OGLRenderer::SetShaderLight(const Light &l) {
	...
	glUniform4fv(glGetUniformLocation(currentShader->GetProgram(), "specColour"), 1, (float*)& l.GetSpecColour());

	glUniform4fv(glGetUniformLocation(currentShader->GetProgram(), "emissColour"), 1, (float*)& l.GetEmissColour());

	glUniform1f(glGetUniformLocation(currentShader->GetProgram(), "lightRadius"), l.GetRadius());

	glUniform1f(glGetUniformLocation(currentShader->GetProgram(), "lightAngle"), l.GetCutOff());
}
#endif
